...
def load_movies_data() -> pd.DataFrame:
    ...
    if settings.use_aws_s3 and aws_manager.is_aws_enabled():
        ...
        try:
            logger.info(
                f"Loading movies data from AWS S3: s3://{settings.aws_s3_bucket}/movies.csv"
            )

            key = (
                f"{settings.aws_s3_data_prefix}movies.csv"
                if settings.aws_s3_data_prefix
                else "movies.csv"
            )
...
def load_ratings_data() -> pd.DataFrame:
    ...
    if settings.use_aws_s3 and aws_manager.is_aws_enabled():
        ...
        try:
            logger.info(
                f"Loading ratings data from AWS S3: s3://{settings.aws_s3_bucket}/ratings.csv"
            )

            key = (
                f"{settings.aws_s3_data_prefix}ratings.csv"
                if settings.aws_s3_data_prefix
                else "ratings.csv"
            )
            df = aws_manager.load_csv_from_s3(settings.aws_s3_bucket, key)
...
def check_data_availability() -> dict:
    ...
    if settings.use_aws_s3 and aws_manager.is_aws_enabled():
        ...
        status["s3_movies"] = aws_manager.check_s3_file_exists(
            settings.aws_s3_bucket, movies_key
        )
        status["s3_ratings"] = aws_manager.check_s3_file_exists(
            settings.aws_s3_bucket, ratings_key
        )
