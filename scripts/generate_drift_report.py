...
def generate_report(
    reference_data: pd.DataFrame, current_data: pd.DataFrame, output_path: Path
):
    ...
    report = Report(
        metrics=[
            ...
            DataQualityPreset(),
            TargetDriftPreset()
            if "rating" in reference_data.columns
            else DataQualityPreset(),
        ]
    )
    ...
    try:
        ...
        for metric in metrics:
            if "result" in metric and "drift_score" in metric["result"]:
                ...
                if drift_score > 0.15:
                    logger.warning(
                        "⚠️  HIGH DRIFT DETECTED! Consider retraining model."
                    )
